...
export async function GET() {
  ...
  const [emailMap, regByLanding, viewsByLanding, bannerByUser] = await Promise.all([
    resolveOwnerEmails([...byOwner.keys()]),
    countGrouped(admin, "registrations", allLandingIds),
    countGrouped(admin, "landing_views", allLandingIds),
    bannerEventsByUser(admin),
  ]);
...
async function bannerEventsByUser(
  admin: ReturnType<typeof getSupabaseAdmin>
): Promise<Map<string, number>> {
  const map = new Map<string, number>();
  const { data } = await admin
